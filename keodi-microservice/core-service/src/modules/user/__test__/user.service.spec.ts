...
describe('UserService', () => {
    ...
    it('calls prisma.user.create with correct data', async () => {
      ...
      expect(mockPrismaService.user.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ id: 'u1', username: 'john' }),
        }),
      );
    });
  });
  ...
  describe('updateProfile', () => {
    it('throws RpcException when user not found', async () => {
      ...
      await expect(service.updateProfile('missing', {} as any)).rejects.toThrow(
        RpcException,
      );
    });
    ...
    it('throws RpcException when phone number is already in use by another user', async () => {
      ...
      await expect(
        service.updateProfile('u1', { phoneNumber: '0900000000' } as any),
  ...
  describe('onBoarding', () => {
    it('throws RpcException when user not found', async () => {
      ...
      await expect(service.onBoarding('missing', ['cat-1'])).rejects.toThrow(
        RpcException,
      );
    });
  ...
  describe('syncUsername', () => {
    it('upserts user and returns USERNAME_SYNCED', async () => {
      ...
      const result = (await service.syncUsername({
        userId: 'u1',
        username: 'newname',
      })) as any;
  ...
  describe('getOtherProfile', () => {
    it('throws NOT_FOUND when target user does not exist', async () => {
      ...
      await expect(
        service.getOtherProfile('viewer', 'missing'),
      ).rejects.toThrow(RpcException);
    });
  ...
  describe('searchOthers', () => {
    it('returns empty result when keyword is blank', async () => {
      const result = (await service.searchOthers({
        userId: 'u1',
        keyword: '  ',
        page: 1,
        limit: 10,
