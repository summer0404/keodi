...
const mockKafkaService = { getClient: jest.fn().mockReturnValue(mockKafkaClient) };
...
describe('ReviewService', () => {
  ...
  describe('create', () => {
    ...
    it('creates review without sentiment analysis when no text', async () => {
      ...
      mockPrismaService.place.findUnique.mockResolvedValue({ id: 'p1', name: 'Place', ownerId: null });
      mockPrismaService.$transaction.mockImplementation(async (fn: any) =>
        fn({ review: { create: jest.fn().mockResolvedValue({ id: 'rev-1' }) } }),
      );
    ...
    it('emits low-rating notification when rating <= 2 and place has owner', async () => {
      const user = { id: 'u1', lastName: 'Doe', firstName: 'John', pictureUrl: null };
      mockPrismaService.user.findUnique.mockResolvedValue(user);
      mockPrismaService.place.findUnique.mockResolvedValue({ id: 'p1', name: 'Place', ownerId: 'owner-1' });
      mockPrismaService.$transaction.mockImplementation(async (fn: any) =>
        fn({ review: { create: jest.fn().mockResolvedValue({ id: 'rev-1' }) } }),
      );

      await service.create({ userId: 'u1', placeId: 'p1', rating: 2, text: null } as any);
    ...
    it('does not emit low-rating notification when rating >= 3', async () => {
      ...
      await service.create({ userId: 'u1', placeId: 'p1', rating: 3, text: null } as any);

      expect(mockKafkaClient.emit).not.toHaveBeenCalledWith(NotificationTopics.ReviewLowRating, expect.anything());
    });
  });
  ...
  describe('getByPlaceId', () => {
    ...
    it('returns reviews with pagination metadata excluding hidden', async () => {
      ...
      const result = await service.getByPlaceId({ placeId: 'p1', page: 1, limit: 10, sortBy: 'createdAt', sortOrder: 'desc' } as any) as any;

      expect(result.reviews).toHaveLength(1);
      expect(result.total).toBe(1);
      expect(mockPrismaService.review.findMany).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ hidden: false }) }));
    });
  });
  ...
  describe('getOwnerReviews', () => {
    it('returns empty when owner has no places', async () => {
      ...
      const result = await service.getOwnerReviews({ ownerId: 'o1', page: 1, limit: 10, sortOrder: 'desc' } as any) as any;
      expect(result.reviews).toHaveLength(0);
      expect(result.total).toBe(0);
    });

    it('returns paginated reviews for owner places', async () => {
      mockPrismaService.place.findMany.mockResolvedValue([{ id: 'p1' }, { id: 'p2' }]);
      mockPrismaService.review.findMany.mockResolvedValue([{ id: 'rev-1' }]);
      mockPrismaService.review.count.mockResolvedValue(1);
      ...
      expect(mockPrismaService.review.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: expect.objectContaining({ placeId: { in: ['p1', 'p2'] } }) }),
      );
    });
    ...
    it('applies responded=true filter', async () => {
      ...
      await service.getOwnerReviews({ ownerId: 'o1', page: 1, limit: 10, sortOrder: 'asc', responded: true } as any);

      expect(mockPrismaService.review.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: expect.objectContaining({ ownerResponse: { not: null } }) }),
      );
    });
  });
  ...
  describe('respondToReview', () => {
    it('throws NOT_FOUND when review does not exist', async () => {
      ...
      await expect(service.respondToReview({ reviewId: 'r1', ownerId: 'o1', text: 'Hi' })).rejects.toThrow(RpcException);
    });

    it('throws FORBIDDEN when owner does not own the place', async () => {
      mockPrismaService.review.findUnique.mockResolvedValue({ id: 'r1', ownerResponse: null, place: { ownerId: 'other-owner' } });
      await expect(service.respondToReview({ reviewId: 'r1', ownerId: 'o1', text: 'Hi' })).rejects.toThrow(RpcException);
    });

    it('throws CONFLICT when review already has a response', async () => {
      mockPrismaService.review.findUnique.mockResolvedValue({ id: 'r1', ownerResponse: 'existing', place: { ownerId: 'o1' } });
      await expect(service.respondToReview({ reviewId: 'r1', ownerId: 'o1', text: 'Hi' })).rejects.toThrow(RpcException);
    });

    it('saves response with ownerRespondedAt', async () => {
      mockPrismaService.review.findUnique.mockResolvedValue({ id: 'r1', ownerResponse: null, place: { ownerId: 'o1' } });
      mockPrismaService.review.update.mockResolvedValue({});

      const result = await service.respondToReview({ reviewId: 'r1', ownerId: 'o1', text: 'Thank you!' }) as any;

      expect(mockPrismaService.review.update).toHaveBeenCalledWith({
        where: { id: 'r1' },
        data: expect.objectContaining({ ownerResponse: 'Thank you!', ownerRespondedAt: expect.any(Date) }),
      });
  ...
  describe('updateResponse', () => {
    it('throws CONFLICT when review has no existing response', async () => {
      mockPrismaService.review.findUnique.mockResolvedValue({ id: 'r1', ownerResponse: null, place: { ownerId: 'o1' } });
      await expect(service.updateResponse({ reviewId: 'r1', ownerId: 'o1', text: 'New' })).rejects.toThrow(RpcException);
    });

    it('updates response and sets ownerResponseEditedAt', async () => {
      mockPrismaService.review.findUnique.mockResolvedValue({ id: 'r1', ownerResponse: 'old', place: { ownerId: 'o1' } });
      mockPrismaService.review.update.mockResolvedValue({});

      const result = await service.updateResponse({ reviewId: 'r1', ownerId: 'o1', text: 'Updated!' }) as any;

      expect(mockPrismaService.review.update).toHaveBeenCalledWith({
        where: { id: 'r1' },
        data: expect.objectContaining({ ownerResponse: 'Updated!', ownerResponseEditedAt: expect.any(Date) }),
      });
  ...
  describe('deleteResponse', () => {
    it('throws CONFLICT when review has no response to delete', async () => {
      mockPrismaService.review.findUnique.mockResolvedValue({ id: 'r1', ownerResponse: null, place: { ownerId: 'o1' } });
      await expect(service.deleteResponse({ reviewId: 'r1', ownerId: 'o1' })).rejects.toThrow(RpcException);
    });

    it('clears response fields on delete', async () => {
      mockPrismaService.review.findUnique.mockResolvedValue({ id: 'r1', ownerResponse: 'hi', place: { ownerId: 'o1' } });
      mockPrismaService.review.update.mockResolvedValue({});

      const result = await service.deleteResponse({ reviewId: 'r1', ownerId: 'o1' }) as any;

      expect(mockPrismaService.review.update).toHaveBeenCalledWith({
        where: { id: 'r1' },
        data: { ownerResponse: null, ownerRespondedAt: null, ownerResponseEditedAt: null },
      });
  ...
  describe('flagReview', () => {
    it('throws CONFLICT when review already has a flag', async () => {
      mockPrismaService.review.findUnique.mockResolvedValue({ id: 'r1', flagStatus: ReviewFlagStatus.PENDING, place: { ownerId: 'o1' } });
      await expect(service.flagReview({ reviewId: 'r1', ownerId: 'o1', reason: 'SPAM' as any })).rejects.toThrow(RpcException);
    });

    it('sets flagReason and flagStatus PENDING', async () => {
      mockPrismaService.review.findUnique.mockResolvedValue({ id: 'r1', flagStatus: null, place: { ownerId: 'o1' } });
      mockPrismaService.review.update.mockResolvedValue({});

      const result = await service.flagReview({ reviewId: 'r1', ownerId: 'o1', reason: 'SPAM' as any }) as any;
  ...
  describe('approveFlags', () => {
    it('throws CONFLICT when no pending flag exists', async () => {
      mockPrismaService.review.findUnique.mockResolvedValue({ id: 'r1', flagStatus: ReviewFlagStatus.REJECTED, place: { ownerId: 'o1', name: 'P' } });
      await expect(service.approveFlags({ reviewId: 'r1' })).rejects.toThrow(RpcException);
    });

    it('sets hidden=true and flagStatus=APPROVED, emits notification', async () => {
      mockPrismaService.review.findUnique.mockResolvedValue({ id: 'r1', flagStatus: ReviewFlagStatus.PENDING, place: { ownerId: 'o1', name: 'Place' } });
      mockPrismaService.review.update.mockResolvedValue({});
