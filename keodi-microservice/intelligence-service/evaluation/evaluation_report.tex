...
% --- Sentiment defaults (overridden by sentiment_metrics.tex)
...
\subsubsection*{Tập dữ liệu mẫu (Golden Set)}

Chúng tôi xây dựng tập 30 đánh giá tiếng Việt được gán nhãn thủ công, bao gồm:
\begin{itemize}
  \item Các đánh giá về từng thuộc tính như:
        \texttt{SERVICE\_QUALITY}, \texttt{EXPENSIVENESS}, \texttt{NOISE\_INTENSITY},
        \texttt{DELICIOUSNESS}, \texttt{RESTROOM\_HYGIENE}.
  \item Các đánh giá có nhiều thuộc tính cùng lúc.
  \item Khả năng hiểu ý ngầm (ví dụ: "đắt" tương ứng với điểm \textsc{Expensiveness} cao).
\end{itemize}

Thang điểm được quy định từ $[-1.0, +1.0]$ và tỉ lệ thuận với ý nghĩa của thuộc tính
(ví dụ: \texttt{NOISE\_INTENSITY} $= +1$ là rất ồn, $-1$ là rất yên tĩnh).

\paragraph{Chỉ số đánh giá:}
\begin{itemize}
  \item \textbf{F$_1$-score:} Đo lường khả năng bắt đúng các thuộc tính xuất hiện trong câu.
  \item \textbf{MAE (Sai số trung bình):} Đo độ lệch giữa điểm số model dự đoán và điểm số thực tế.
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Kết quả tổng thể phân tích cảm xúc (30 đánh giá mẫu)}
\label{tab:sentiment_overall}
\begin{tabular}{lc}
\toprule
\textbf{Chỉ số} & \textbf{Giá trị} \\
\midrule
Precision (Độ chính xác)         & \SentPrecision{} \\
Recall    (Độ bao phủ)           & \SentRecall{} \\
F$_1$-score                      & \SentFOne{} \\
\midrule
MAE  (Sai số trung bình)         & \SentMAE{} \\
RMSE (Căn bù phương sai sai số)  & \SentRMSE{} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Kết quả chi tiết theo từng thuộc tính}
...
\midrule
\textbf{Trung bình} & \SentPrecision{} & \SentRecall{} & \SentFOne{} & \SentMAE{} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{Về việc nhận diện thuộc tính ($F_1$\,=\,\SentFOne{}):}
        Model cho kết quả khá tốt, nhận diện đúng hầu hết các yếu tố được nhắc đến trong đánh giá. Một số trường hợp bỏ sót thường rơi vào các câu nói quá ẩn ý.

  \item \textbf{Về độ chính xác điểm số (MAE\,=\,\SentMAE{}):}
        Sai số trung bình ở mức thấp (\SentMAE{}). Điều này cho thấy model hiểu khá tốt sắc thái khen chê của người dùng và đưa ra điểm số sát với thực tế.
\end{itemize}
...
\multirow{4}{*}{\model{}}
  ...
  & 95\% CI (NDCG@5)               & [\RankSynLGBNDCGFiveLo{}--\RankSynLGBNDCGFiveHi{}]
                                   & Bootstrap 1\,000 lần \\
\midrule
\multirow{2}{*}{LLM Sentiment}
  & $F_1$-score (nhận diện thuộc tính) & \SentFOne{}     & 30 reviews mẫu \\
  & MAE (độ lệch điểm số)              & \SentMAE{}      & Thang $[-1,+1]$ \\
\bottomrule
\end{tabular}
\end{table}
