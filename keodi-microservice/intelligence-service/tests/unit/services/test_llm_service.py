...
class TestLLMService:
    ...
    def test_llm_service_initialization_with_groq(
        ...
        mock_settings = MagicMock(llm_deployment_mode="groq", groq_api_key="test-key")
        with patch('app.services.llm.llm_service.get_settings', return_value=mock_settings):
    ...
    def test_llm_service_initialization_no_providers(
        ...
        mock_settings = MagicMock(llm_deployment_mode="groq", groq_api_key=None, modal_endpoint=None)
        with patch('app.services.llm.llm_service.get_settings', return_value=mock_settings):
            mock_prompts.return_value = MagicMock()
            mock_embedding.return_value = MagicMock()
    ...
    async def test_extract_user_intent_success(
        ...
        mock_settings = MagicMock(
            llm_deployment_mode="groq", groq_api_key="test-key", llm_max_retries=1
        )
        with patch('app.services.llm.llm_service.get_settings', return_value=mock_settings):
    ...
    async def test_extract_user_intent_provider_not_found(
        ...
        mock_settings = MagicMock(
            llm_deployment_mode="groq", groq_api_key="test-key", modal_endpoint=None, llm_max_retries=1
        )
        with patch('app.services.llm.llm_service.get_settings', return_value=mock_settings):
            ...
            service = LLMService()
            # Switch mode to something unavailable
            service.mode = "modal"
    ...
    async def test_extract_user_intent_with_retries(
        ...
        mock_settings = MagicMock(
            llm_deployment_mode="groq", groq_api_key="test-key", llm_max_retries=3
        )
        with patch('app.services.llm.llm_service.get_settings', return_value=mock_settings):
            mock_provider = AsyncMock()
    ...
    async def test_sentiment_analysis_success(
        ...
        with patch('app.services.llm.llm_service.get_settings', return_value=mock_settings):
            ...
            attr1 = MagicMock()
            attr1.name = "cleanliness"
            attr2 = MagicMock()
            attr2.name = "service"
            mock_attr_repo.get_all_attributes = AsyncMock(return_value=[attr1, attr2])
    ...
    async def test_llm_service_start(
        ...
        mock_settings = MagicMock(llm_deployment_mode="groq", groq_api_key="key")
        with patch('app.services.llm.llm_service.get_settings', return_value=mock_settings):
            mock_prompts.return_value = MagicMock()
            mock_embedding.return_value = MagicMock()
