...
class TestRankingService:
    ...
    async def test_ranking_service_start(self, mock_repo):
        ...
        with patch('app.services.ranking.ranking_service.settings') as mock_settings:
            mock_settings.ltr_model_path = "/tmp/test_model.txt"
    ...
    async def test_ranking_no_model(self, mock_repo):
        ...
        with patch('app.services.ranking.ranking_service.settings') as mock_settings:
            ...
            with patch('app.services.ranking.ranking_service.lgb.Booster'):
                ...
                assert len(result) == 2
                # Changed expectation to ai_score based on current implementation returning ai_score: 0.0 when no model
                # Actually implementation returns: [{"place_id": pid, "ai_score": 0.0} for pid in place_ids]
    ...
    async def test_ranking_with_model(self, mock_repo):
        ...
        with patch('app.services.ranking.ranking_service.settings') as mock_settings:
            ...
            with patch('app.services.ranking.ranking_service.lgb.Booster') as mock_booster:
                service = RankingService()
                service.model = MagicMock()
